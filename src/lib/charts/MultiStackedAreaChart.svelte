<script lang="ts">
    ...
    // Log to check if values are non-zero
    console.log('Data for Chart:', data);

    // Prepare data for chart without scaling to billions to check visibility
    const filteredData = data.filter(d => d.date >= new Date('2020-01-01') && d.date <= new Date('2024-05-01'));

    // Log the filtered data to make sure it is correct
